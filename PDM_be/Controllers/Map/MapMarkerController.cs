...
namespace PDM_be.Controllers
{
    ...
    public class MapMarkerController : BaseApiCRUDController<DbSession,MapMarker, int>
    {
        private readonly IDbFactory _dbFactory;
        public MapMarkerController(IDbFactory dbFactory, IRepository<MapMarker, int> repository)
        ...
        public async Task<IActionResult> DatatableAsync([FromBody] TableParameters dataTb)
        {
            ...
            string tableAlias = typeof(MapMarker).Name.ToLower();
            string order = $" 1 ASC";
            List<MapMarker> data;

            var recordsTotal = await session.Connection.CountAsync<MapMarker>(stm => stm
            ...
            {
                var tableName = RepoDBHepler.OnlyTableName<MapMarker>();
            ...
            if (dataTb?.length == -1)
            {
                data = (await session.Connection.FindAsync<MapMarker>(stm => stm
            ...
            return Ok(new RestPagedDataTable<IEnumerable<MapMarker>>
            {
                data = data,
                recordsFiltered = await session.Connection.CountAsync<MapMarker>(stm => stm
                    .WithAlias(tableAlias)
                    .Where($"{condition}")
                    .WithParameters(withParams)
                ),
