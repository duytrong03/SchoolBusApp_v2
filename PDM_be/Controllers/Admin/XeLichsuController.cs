...
namespace PDM_be.Controllers
{
    ...
    [Route("api/xe-lichsu")]
    public class XeLichsuController : BaseApiCRUDController<DbSession, XeLichsu, int>
    {
        public XeLichsuController(IDbFactory dbFactory, IRepository<XeLichsu, int> repository)
        ...
        public async Task<IActionResult> DatatableAsync([FromBody] TableParameters dataTb)
        {
            ...
            string tableAlias = typeof(XeLichsu).Name.ToLower();
            string order = $" 1 ASC";
            List<XeLichsu> data;

            var recordsTotal = await session.Connection.CountAsync<XeLichsu>(stm => stm
            ...
            {
                var tableName = RepoDBHepler.OnlyTableName<XeLichsu>();
            ...
            if (dataTb?.length == -1)
            {
                data = (await session.Connection.FindAsync<XeLichsu>(stm => stm
                    .Include<XeLichsu>(x => x.LeftOuterJoin())
            ...
            return Ok(new RestPagedDataTable<IEnumerable<XeLichsu>>
            {
                data = data,
                recordsFiltered = await session.Connection.CountAsync<XeLichsu>(stm => stm
                    .WithAlias(tableAlias)
                    .Where($"{condition}")
                    .WithParameters(withParams)
                ),
