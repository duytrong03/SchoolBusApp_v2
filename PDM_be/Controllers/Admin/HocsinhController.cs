...
namespace PDM_be.Controllers
{
    ...
    public class HocsinhController : BaseApiCRUDController<DbSession,Hocsinh, int>
    {
        ...
        public async Task<IActionResult> DatatableAsync([FromBody] TableParameters dataTb)
        {
            ...
            string tableAlias = typeof(Hocsinh).Name.ToLower();
            string order = $" 1 ASC";
            List<Hocsinh> data;

            var recordsTotal = await session.Connection.CountAsync<Hocsinh>(stm => stm
            ...
            if (dataTb?.length == -1)
            {
                data = (await session.Connection.FindAsync<Hocsinh>(stm => stm
            ...
            return Ok(new RestPagedDataTable<IEnumerable<Hocsinh>>
            {
                data = data,
                recordsFiltered = await session.Connection.CountAsync<Hocsinh>(stm => stm
                    .WithAlias(tableAlias)
                    .Where($"{condition}")
                    .WithParameters(withParams)
                ),
